...
#terminate all the past messages, so that the poll only involves the common times
def cleanup(sessions, chat_id):
    session = sessions.pop(chat_id)
    if session['poll']:
        api.tgStopPoll(chat_id, session['poll'])

#we made an extensive function that just structures the flow of the bot 
def run():
    prev_update_id = None
    sessions = {}
        ...
        try: #try and except to catch any errors/bugs
            ...
            if chat_id in sessions:
                ...
                if step == 1:
                    ...
                    if (current_last_msg in generateTimeList()):
                        ...
                        if sender_username in preferred_times:
                            ...
                            #what is removed is the time picked by the users since they already chose it 
                ...
                if descCmd: #command to add the optional description option in the Google Calendar event
                    ...
                    continue
                if current_last_msg=='/cancel': #if the booking is cancelled, all the messages are erased in the process so that users can start with a clean slate
